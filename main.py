...
def callback_data(call):
    ...
    if result["situation"]:
        ...
        bot.send_message(call.message.chat.id, response(result) + text_water + str(result1)[:5:] + " литров", reply_markup=generator_menu(stop_menu_list))
    else:
...
def startWM(message):
    if message.text.isdigit():
        ...
        if result["situation"]:
            ...
            result1 = a.transfer()
            result1 = result1["score"] / 400
            bot.send_message(message.chat.id, response(result) + text_water + str(result1)[:5:] + " литров", reply_markup=generator_menu(stop_menu_list))
